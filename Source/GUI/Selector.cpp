...
namespace sf
{
    Selector::Selector(sf::Vector2f position,sf::Vector2f size, sf::Color color,sf::Color buttonColor, unsigned int var, unsigned int maxVar,std::string title,std::string f,float percentageFontSize, sf::Color textColor, sf::Text::Style style)
    {
        font=f;
    ...
    sf::Vector2f Selector::getPosition()
    {
        return leftButton.getPosition();
    }

    sf::Vector2f Selector::getSize()
    {
        return sf::Vector2f(leftButton.getSize().x+body.getSize().x+rightButton.getSize().x,body.getSize().y);
    }

    unsigned int Selector::getValue()
    {
        return value;
    }

    void Selector::setValue(unsigned int var)
    {
        value=var;
    }
    ...
    void Selector::centerResolutionWidth()
    {
